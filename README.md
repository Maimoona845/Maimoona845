# Hi, I'm Maimoona Ali 👋

### 🧬 About Me
I am a **Bioinformatics Specialist** with 10+ years of experience, currently focused on **Lung Cancer Precision Medicine**. My work bridges the gap between large-scale transcriptomic data and experimental validation.

### 🚀 Key Research Focus
* **Transcriptomics:** RNA-seq analysis of EGFR-mutant NSCLC.
* **Machine Learning:** Deep Learning models for miRNA target prediction.
* **Precision Medicine:** Identifying biomarkers for TKI (Osimertinib) resistance.

### 🛠 Tech Stack
* **Languages:** Python (Keras, TensorFlow), R (DESeq2), MATLAB.
* **Tools:** Galaxy Project, BLAST, RNAhybrid, Linux/Bash.
* **Wet-Lab:** Western Blotting, RT-qPCR, PCR, Cloning.

### 📬 Connect with Me
* **LinkedIn:** [https://www.linkedin.com/in/maimoona-ali-bioinformatics/]
* **Email:** alimaimoona37@gmail.com
